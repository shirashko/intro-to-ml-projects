...
class L2(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        """
        return np.dot(self.weights, self.weights_)
...
class L1(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        return np.linalg.norm(self.weights, ord=1)  # np.sum(np.abs(self.weights))
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        weightedX = X @ self.weights
        return -np.sum(y * weightedX - np.log(1 + np.exp(weightedX))) / len(X)
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        sigmoid_vals = 1 / (1 + np.exp(-(X @ self.weights)))  # Compute sigmoid values

        # Compute the gradient using the formula: -(1/m) * sum_i^m[(y_i - sigmoid(<x_i, w>)) * x_i]
        gradient = - (X.T @ (y - sigmoid_vals)) / len(X)
...
class RegularizedModule(BaseModule):
    ...
    def weights(self, weights: np.ndarray) -> None:
        ...
        if self.include_intercept_:
            temp[0] = 0
        self.regularization_module_.weights = temp

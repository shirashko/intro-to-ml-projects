...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.include_intercept_:
            X = np.c_[np.ones(len(X)), X]  # Adding column of ones to the left of X for the intercept

        n_features = X.shape[1]
        # chose randomly for starting point of the gradiant descent algorithm
        init_weights = np.random.randn(n_features) / np.sqrt(n_features)

        if self.penalty_ == "l1":
            model = RegularizedModule(LogisticModule(init_weights), L1(init_weights), weights=init_weights,
                                      lam=self.lam_, include_intercept=self.include_intercept_)
        elif self.penalty_ == "l2":
            model = RegularizedModule(LogisticModule(init_weights), L2(init_weights), lam=self.lam_,
                                      weights=init_weights, include_intercept=self.include_intercept_)
        else:
            model = LogisticModule(weights=init_weights)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        return sigmoid_function(X @ self.coefs_)
